...
const DriverWaitlist = () => {
  ...
  const checkRegistrationStatus = async () => {
    try {
      ...
      if (error && error.code !== 'PGRST116') {
        ...
        setRegistrationEnabled(false); // افتراض أن التسجيل معطل
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.driver_name.trim() || !formData.phone_number.trim() || !formData.whatsapp_number.trim()) {
    ...
    try {
      ...
      toast.success(isRTL ? 'تم تسجيلك في قائمة الانتظار بنجاح!' : 'You have been added to the waitlist successfully!');
      navigate('/');
    } catch (error) {
      const errorMessage = handleDatabaseError(error, isRTL);
      toast.error(errorMessage);
        ...
        <Card className="w-full max-w-md">
          ...
          <CardContent>
            <p className="text-center text-gray-600 mb-4">
              {isRTL 
                ? 'تسجيل السائقين الجدد متاح حالياً. يمكنك التسجيل مباشرة.'
                : 'New driver registration is currently available. You can register directly.'
              }
            </p>
            <Button 
        ...
        <Card className="max-w-2xl mx-auto">
          <CardHeader>
            <CardTitle className="text-2xl font-bold text-center">
              {isRTL ? 'قائمة انتظار السائقين' : 'Driver Waitlist'}
            </CardTitle>
