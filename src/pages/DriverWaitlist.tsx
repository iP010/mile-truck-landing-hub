...
export default function DriverWaitlist() {
  const [waitlist, setWaitlist] = useState<DriverWaitlistItem[]>([]);
  ...
  const fetchWaitlist = async () => {
    try {
      ...
      toast.error('خطأ في تحميل قائمة انتظار السائقين');
    } finally {
      setLoading(false);
    }
  };
  ...
  const deleteFromWaitlist = async (id: string) => {
    if (!confirm('هل أنت متأكد من حذف هذا السائق من قائمة الانتظار؟')) return;
    ...
    try {
      ...
      toast.success('تم حذف السائق من قائمة الانتظار');
      fetchWaitlist();
    } catch (error) {
      console.error('Error deleting from waitlist:', error);
      toast.error('خطأ في حذف السائق من قائمة الانتظار');
    }
  };
          ...
          <div className="container mx-auto p-6 space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                />
                <h1 className="text-3xl font-bold text-gray-800">قائمة انتظار السائقين</h1>
              </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <Badge variant="secondary" className="px-3 py-1">
                إجمالي السائقين في الانتظار: {waitlist.length}
              </Badge>
            </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle>السائقين في قائمة الانتظار</CardTitle>
              </CardHeader>
              <CardContent>
                {filteredWaitlist.length === 0 ? (
                  <p className="text-center text-gray-500 py-8">لا توجد سائقين في قائمة الانتظار</p>
                ) : (
